...
Voor deze workflow hebben werd de Clarifai handleiding gevolgd. %TODO voetnoot
Nadat het model klaar was, werd dit model losgelaten op alle 845 beelden van Huis van Alijn zoals in sectie \ref{subsec:workflow1}.

\subsubsection{Beelden voorzien van een label opladen naar de Computer Vision API}
\label{subsubsec:beelden-met-label-opladen}
...
\subsubsection{Het model creëren}
\label{subsubsec:model-creëren}
...
Taggen van validatieset via workflow uit sectie \ref{subsec:workflow1}. Het valideren gebeurt vergelijkbaar met sectie \ref{subsubsec:gegevens-valideren}.